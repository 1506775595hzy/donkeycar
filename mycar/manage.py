...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    ########################################################
    # adding extra paramter
    class Extra_control:
        def __init__(self):
            self.extra = 0
        def run_threaded(self):
            from donkeycar.parts.us100 import Driver
            distance = Driver(pi,23,24).read()
            time.sleep(0.03)
            if(distance > 20):
                extra = 0
            else:
                extra = 1# when itteration is nearby
            return extra

        def update(self):
            pass
    V.add(Extra_control(),outputs=['extra'],threaded=True)
    #this is extra paramter to control the model we get
...
#########################################################
    ...
    class DriveMode:
        def run(self, mode,extra,
            ...
            if mode == 'user' or extra == 1:
                return user_angle, user_throttle

            elif mode == 'local_angle':#extra == 1 means nearby
